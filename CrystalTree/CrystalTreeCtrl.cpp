...
void CCrystalTreeCtrl::DrawItem(HDC hdc)
{
	...
	while(hItem)
	{
		...
		}else
		{
			...
			{
				COLORREF c=::SetBkColor(hdc, 0xFF7CFF);
			...
			if(uState&TVIS_SELECTED)
			{
				//HBRUSH hH = CreateSolidBrush(::GetFocus()==m_hWnd?0xFFFFFF:(::GetSysColor(COLOR_BTNFACE)));
				//FillRect(hdc, &rc, hH);
				//DeleteObject(hH);
				COLORREF c=::SetBkColor(hdc, 0xEEEEEE);
				ExtTextOut(hdc, 0, 0, OPAQUE, &rc, NULL, 0, NULL);
				::SetBkColor(hdc, c);
			}
		}
...
HBRUSH CCrystalTreeCtrl::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if(hItem && (pWnd->m_hWnd==m_Edit.m_hWnd || m_hwndItem==pWnd->m_hWnd))
	{
		...
		hbr = CreateSolidBrush(0xFF9CFF);//::GetSysColor(COLOR_WINDOW)
		pDC->SetTextColor(0xFFFFFF);
		pDC->SetBkColor(0xFF9CFF);
...
#define swp (SWP_SHOWWINDOW|SWP_NOACTIVATE|SWP_NOOWNERZORDER|SWP_NOREPOSITION|SWP_NOSENDCHANGING|SWP_NOZORDER)
		...
		UINT ct = pData->nStyle&DATATYPE_MASK;
		if(ct>=CTCS_BYTE && ct<=CTCS_UINT && !(pData->nStyle&CTCS_HEXMODE))
			CWnd::ModifyStyle(m_hwndItem, 0, ES_NUMBER, 0);
		else
			CWnd::ModifyStyle(m_hwndItem, ES_NUMBER, 0, 0);
	...
	case WM_KEYDOWN:
		{
			...
			case VK_SPACE:
				{
					if(::GetFocus()==m_hWnd && ActiveLink(GetSelectedItem()) || wKey==VK_RETURN)
						return TRUE;
				}break;
			}
		}break;
...
void CCrystalTreeCtrl::SetVarData(HTREEITEM hItem, CString &strData)
{
	if(hItem)
	{
		...
		if(p)
		{
			...
			{
				LPTSTR s = sFindChar(p->pData, _T(';'));
				if(s) *s=0;
				si.Format(_T("%s;"), p->pData);
			}
